...
import { Link, Outlet, useLocation, useNavigate } from 'https://esm.sh/react-router-dom@7.11.0';
...
const AdminLayout: React.FC = () => {
  ...
  useEffect(() => {
    // Check if user is already in local storage (for custom db users)
    const savedUser = localStorage.getItem('drishti_user');
    if (savedUser) {
      setUser(JSON.parse(savedUser));
    }

    // Sync with Firebase Auth state if applicable, but don't force logout if local user exists
